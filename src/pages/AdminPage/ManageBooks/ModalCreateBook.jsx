...
import { handleCreateNewBook, handleCreateNewUser, handleUploadFile } from '../../../service/api';
...
const ModalCreateBook = (props) => {
    const { isModalOpen, setIsModalOpen, getAllBook, category } = props
    ...
    const onFinish = async (values) => {
        ...
        let res = await handleCreateNewBook(author, mainText, price, quantity, sold, category, thumbnail[0].urlUpload, imgSlider)
        if (res && res.data) {
    ...
    return (
        <>
            <Modal width={'60%'} title="Add new book" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
                <Form
                    name="from-add-book"
                    style={{ marginTop: 50 }}
                    onFinish={onFinish}
